...
var kkk = 0;
    ...
    function addThumbnail(imageField, mName, counter){
        ...
        $.ajax({
            url: imageField.resourceURL + ".infinity.json",
            cache: false
        }).done(handler);
        ...
        function handler(data){
            ...
            var componentType = data[mName][rowString]['components'],
                fileRef = "";

            if(componentType == "imageandtext") {
				fileRef = data[mName][rowString]['par2']['imagerow']['par2']['imageandtext']['fileReference'][0];
            }

            if(componentType == "twoimages") {
                if(fRef == 'fileReference1') {
                    var parString = 'par';
					fileRef = data[mName][rowString]['par2']['row2cellsequal'][parString]['image']['fileReference'];
                }
                if(fRef == 'fileReference2') {
                    var parString = 'par2';
					fileRef = data[mName][rowString]['par2']['row2cellsequal'][parString]['image']['fileReference'];
                }
            }
            ...
            if(componentType == "pulloutimage") {
				fileRef = data[mName][rowString]['par2']['row2cells13']['par']['image']['fileReference'][0];
            }
